...
void test( bool in_place )
{
    ...
    // Everyone needs recv_buf, recv_cnts, recv_offsets (unlike MPI_Gatherv).
    std::vector<int> recv_cnts( mpi_size ), recv_offsets( mpi_size + 1 );
    ...
    recv_offsets[ 0 ] = 0;
    for (int r = 1; r < mpi_size + 1; ++r) {
        recv_offsets[ r ] = recv_offsets[ r-1 ] + recv_cnts[ r-1 ];
    }

    int size = recv_offsets[ mpi_size ];
    ...
    if (in_place) {
        ...
        //--------------------
        mpi_call(
            MPI_Allgatherv( MPI_IN_PLACE, n, MPI_DOUBLE,
                            &recv_buf[0], &recv_cnts[0], &recv_offsets[0],
                            MPI_DOUBLE, comm ) );
    }
    else {
        // Everyone including root sends.
        ...
        //--------------------
        mpi_call(
            MPI_Allgatherv( &send_buf[0], n, MPI_DOUBLE,
                            &recv_buf[0], &recv_cnts[0], &recv_offsets[0],
                            MPI_DOUBLE, comm ) );
    }

    // Everyone receives (unlike MPI_Gatherv).
    printf( "%2d: gather: [ %s ]\n",
            mpi_rank, to_string( recv_buf ).c_str() );
}
